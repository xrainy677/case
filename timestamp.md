...
double get_percentiles(const vector<double>& v,int p)
{
    if(p<0||p>100)
    {
        cerr<<"输入错误百分位数"<<endl;
    ...
    double order_number=p/100*(v.size()-1);      //-1对应vector中的下标
...
double get_average(const vector &v){
...
int main()
{
    ...
    for(int i=0;i<101;i++)
    {
        ...
        auto end=timer::get_concrete_time();
